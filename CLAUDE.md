...
- **Multi-line signature-block fills must be split into per-field `replace_all`.** On MSA/SOW (multiple signature blocks) the LLM collapses a block into ONE `replace` whose target is several `label: [__]` lines, all differing (`Signed by: [__]\nTitle: [__]\nfor and on behalf of [__]`). That target is unapplyable — `body.search` can't cross paragraph breaks, so only the first line matches and the 85% completeness guard rejects it ("Couldn't find the exact target text"); it would also only ever fill the FIRST block. NDA worked only because it has one block. Fix: `normalizeProposals` in [parseEditBlocks.ts](clients/word/src/parseEditBlocks.ts) (a) **splits** a multi-line `replace` whose changed lines are ALL labeled blanks into one `replace_all` per field, and (b) **collapses** the LLM's duplicate per-block cards (main + appendix) into one fill-every `replace_all`. `replace_all` (not `replace`) because the blanks recur across blocks and `replaceAll` snapshots all matches in one pass (no struck-text re-find). Proven by traces `02e41ead` (MSA) / `ce45b899` (SOW). Principle-based, model-neutral — no prompt change.
