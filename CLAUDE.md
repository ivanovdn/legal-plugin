...
- **Chat path is grounded + remembers reviews:** `skills/grounding.py` (shared with `contract_review`) attaches the playbook bundle + governing MSA; `memory/review_store.py` (SQLite) persists the markdown review keyed to `memory/document_id.py::resolve_document_id` (preamble hash — interim; Office.js custom-document-property id is the durable upgrade); `legal_research._run_doc_chat` injects the latest review (its `Suggested Redlines` section **stripped** via `_strip_redlines_section` so chat doesn't re-propose fills) and assembles stable-grounding-first / question-last. Restart `bash scripts/start.sh` after changing any of these files.
