...
- **SOW reviews auto-attach the governing MSA.** When `contract_review` detects `contract_type == "sow"` (with uploaded text), it calls `rag/related_docs.py::get_parent_msa(client_id)` (scrolls Qdrant for `doc_type="msa"` + `client_id`), appends a `--- GOVERNING MSA (title) ---` block to the user message (capped at `_MSA_MAX_CHARS`, truncation-marked) and adds `_MSA_COMPARISON_DIRECTIVE` as the **last** system message. Strictly additive: non-SOW, no-MSA-on-file, or any lookup error → standalone review (wrapped in try/except — retrieval must never break the review). The directive is **structural/model-neutral** — it orchestrates the comparison and defers all legal judgment to the playbook (mirrors `sow/SKILL.md:59` + MSA-001 precedence rules; forbids inventing MSA terms), so **SKILL.md is still the ceiling** (same category as the existing `_OUTPUT_CONSTRAINTS`). This *enables* a check the SOW playbook already mandates (`sow/SKILL.md:6` "conflicts with MSA"; `:28` names the MSA as required input) — it does not add a new legal rule. Demo prep: `scripts/ingest_demo_msa.py` ingests `data/Trinetix Model MSA 2025 (3)-1.docx` as `doc_type="msa"` (clears prior MSA chunks first since parsers assign random-UUID doc_ids; run it once, then restart the backend).
