...
> **Post-implementation (review-driven) — the code differs from the Step-1/Step-4 blocks above; those blocks predate the fixes below. The shipped behavior is:**
> - `1bb05e6` (Task-1 review finding): "most-specific-wins per line" — a ref that is a proper substring of a longer ref on the same line is excluded from the drop vote, so a bare label (`[Legal Name]`) recurring across two contexts can't protect a row whose full span is filled.
> - `85d9d98` (final-review #1/#3/#5): (#1) **only single-marker backtick spans are droppable** — a span bundling ≥2 markers (the MSA/SOW `Signed by: [__] / Title: [__] / …` shape) is left intact so a partial fill can't drop a live blocker; (#3) the reconciliation note is built from the tokens whose rows were **actually dropped** (return value is now `dropped_tokens`), and no note is emitted when nothing dropped; (#5) the error-path test asserts `memory_degraded` is not set.
> See the shipped source in `skills/legal_research.py` and `docs/context_and_memory_audit.md` for the authoritative behavior.
