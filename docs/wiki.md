...
| ~~Server-side per-attorney conversation store~~ | ~~Medium~~ | **DONE (adapted)** — shipped in `feat/conversation-store` (slice 2). Chat conversations persist per `(document_id, attorney_id)` in SQLite (`memory/conversation_store.py`), appended per turn by `memory_writer` and read by `_run_doc_chat` (prefers the durable store over Redis `chat_history`, falls back when empty) — additive to Redis, not the originally-sketched VM-keyed `(document_uuid, attorney_id)` render cache. Attorney identity landed as the per-install `localStorage` id below (not O365 SSO — that's still slice 3). **Sideload-smoke-confirmed (2026-07-15)** — see the Shipped row. |
...
| ~~Chat: reconcile a recalled review against the current doc (stale-recall)~~ | ~~Medium~~ | **DONE** — shipped in `feat/stale-recall-reconciliation` (deterministic placeholder reconciliation; see the Shipped row). `_load_prior_review_block` now reconciles the recalled review against the current `uploaded_text` and drops findings whose placeholders were filled after the review, conservatively (never drops a live blocker). **Deferred (not in this branch):** the *re-review nudge* when the doc diverges beyond blank-fills (needs a doc-change signal), and reconciling *substantive* findings via clause-level retrieval (needs structured-JSON review output). **Gate-verdict recomputation shipped** in `feat/gate-verdict-reconciliation` (see the Shipped row). |
