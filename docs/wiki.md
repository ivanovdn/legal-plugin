...
> Last updated: 2026-06-16 | 200 tests + 43 frontend parser asserts passing | Python 3.12 | Redis checkpointer active | Word add-in shipped (chat-driven edits) | Trinetix legal-team playbook integrated | Smoke-tested on real MSA / SOW / BAA | Main-flow audit + bug fixes #1/#2/#3/#4/#5 + placeholder-recall Layer 1 (see docs/output_format_conflict.md)
...
| Deterministic placeholder / signature-block safety-net — Layer 2 | Medium (deferred) | **Layer 1 shipped (prompt cue) — this is the remaining *guarantee*.** Cause settled by A/B: the recall gap was Fix #2 collateral signal loss, **not** the two-column masking originally guessed; restoring the cue deterministically recovers structured detection on the NDA (`PROSE→FULL`, 6/6). But Layer 1 stays LLM-dependent — it repairs the regression and lifts recall, yet **cannot guarantee** it: the 129KB MSA (`f4b383f0`) missed all 6 placeholders (length-driven), and prose-only runs (`01d4e2ae`) never reach Word's `deriveBlockers` blockers card. Layer 2 = a deterministic post-LLM scan of the faithful `uploaded_text` for unfilled markers (`[__]`, `[Legal Name]`, `[Month] [Date], [Year]`, `[Address]`; **exclude** generated-draft `[Source: doc_id]` tags) that **forces** each into a structured Missing-Context row + "Do not send for signature" (Fix #4 then escalates `risk_level`/`requires_attorney`). Must emit **structured rows**, not just gate prose. Deferred per decision — build when Layer 1's measured recall (esp. on long docs) proves insufficient. A first instance of the self-improving harness. |
