...
| **`chat_history` bleed into `contract_review` re-reviews** | **High (was Low — re-rated 2026-08-13, next branch)** | **Harm is no longer theoretical: a re-review silently lost a signature-block blocker.** [graph/nodes/llm_caller.py:59-60](graph/nodes/llm_caller.py) splices `chat_history` between the playbook system message and the output-constraints message; [clients/word/src/App.tsx:12](clients/word/src/App.tsx) mints ONE `session_id` for both tabs, so Chat-tab turns land in the Findings-tab prompt. **Three-run isolation on the same NDA (2026-08-13):** ① no chat, no prefs → 3 Missing-Context rows, signature block present, blockers card 3. ② chat_history (4 Sony turns) + preference → **1** Missing-Context row, **signature block dropped from Key Findings**, blockers card **1** (`deriveBlockers` rebuilds the card from Key Findings, so the unfilled `Signed by: [__]` stopped rendering as a blocker even though the gate prose still named it). ③ fresh session, preference still on → 3 Missing-Context rows **+ 2 Red**, signature block present, blockers card 5 — the strongest of the three. **⇒ `chat_history` is the culprit; the attorney-preference feature is exonerated** (first real evidence the Stage-1 ceiling holds under an additive pref). Caveat: **n=1 per cell** — direction is clear, rate is unknown. **Sharpest point: this injection benefits nothing on any path it reaches.** Doc-chat builds its own history via `_load_prior_conversation`, and `llm_caller` *skips entirely* on chat turns (`legal_research` sets `llm_response` — visible as 0-token `llm_caller` GENERATIONs in the smoke traces), so the splice only ever reaches `contract_review` + the Chainlit-only skills. Fix: skip the splice for `contract_review`, or drop it from `llm_caller` altogether (touches the Chainlit skills — own branch, own verification). |
