...
| Clause-locator hardening for placeholder / short anchors | Medium | Surfaced by the click-to-jump smoke test (2026-07-09) on the model NDA. `findClauseRangeFromAnchors` (used by `goToClause` **and** `showInDocument`/`acceptRedline`) (a) **can't locate** signature-block findings whose anchor is a synthesized label ("Execution Block / Entity") or a bracketed blank `[__]` — `body.search` treats `[]` as wildcards → "Couldn't locate this clause"; and (b) can **mislocate** on short/generic anchors — "Title" matched inside "en**titled**" and commented in the wrong place. Fixes: apply the existing `escapeWordWildcards` + wildcard-mode retry (already in `searchFirst`/`replaceAll`) to the navigation locator; add a minimum-length / word-boundary guard so short anchors can't match mid-word; for label-only Missing-Context findings with no literal doc text, suppress jump/comment or anchor on the nearest real heading. **Pre-existing** locator behavior (not introduced by the Bucket A change) but made far more visible by click-to-jump. |
