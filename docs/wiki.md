...
|---|---|---|
| **VM deployment + hosting (spec 2 of the O365/SharePoint effort)** | High | Companion spec to the just-shipped Postgres store migration (spec 1 — see Shipped row). Covers reverse proxy, TLS cert provisioning, remote Ollama, the production add-in manifest, and sideload/rollout to attorney machines. Spec: `docs/superpowers/specs/2026-07-23-postgres-store-migration-design.md` (§ "Spec 1 of 2"). Blocked on nothing technical — spec 1 was built self-contained and testable on the dev machine precisely so this could follow independently. |
