...
| **Chat edit path fabricates facts into the document (ceiling violation)** | **High (new 2026-08-13)** | Asked only to *"fill the signature block with Suzy Quatro"*, the model also rewrote the Preamble and invented a counterparty address: `Sony Corporation, mailing address at 1-7-1 Konan, Minato-ku, Tokyo 108-0075, Japan`. **The address was never supplied** — not in the document, not in the playbook, not in any user turn (the user said only "my client is Sony"). It is Sony's real HQ, recalled from pretraining, and it arrived as a one-click **Apply** that would have written it into a legal instrument as a tracked change. A worse class than the indemnity-carve-out violation below: that improvised an *opinion* into a review; this fabricates a *fact* into a contract. **Two aggravating primers were present** — the `USER.md` preference ("Always assume the client is Sony Company…") and 2 chat turns naming Sony — so naming an entity in standing memory appears to license the model to fill in what it "knows" about it. Cannot be attributed to the preference alone (chat_history names Sony too — same confound), but it is the second time this pref sits beside surprising behavior and it needs its own eval case. **No guard exists today:** nothing checks whether `new_text` introduces tokens absent from `uploaded_text`. Per [[fix-in-code-not-prompt]] the fix is a code guard (flag/refuse edits introducing unsourced proper nouns, addresses, dates), not prompt tuning. Trace `0f63a143`. Note this run **also** reproduced the known SCOPE over-reach below, and announced it ("I **also** filled in the Preamble…") despite the SCOPE rule being present in the prompt. |
| **MSA truncation produces confident false findings** | **High (new 2026-08-13)** | `config.msa_max_chars` (24 000) cut the 73 152-char governing MSA to **33%** for a SOW chat turn asking *"does this conflict with the governing MSA?"*. The model answered well overall — quoting MSA §2 precedence verbatim (*"in case of any conflict… the provisions of that particular SOW shall prevail"*) — but then reported *"the MSA requires USD payments but **cuts off mid-sentence** regarding procedures, leaving a gap that this SOW fails to clarify."* **That gap is our truncation, not a defect in the MSA.** The model cannot distinguish "this contract is incomplete" from "you showed me a third of it", and the trailing `[MSA truncated to 24000 chars for review]` marker did not prevent the misreading. Answering an MSA-conflict question from a third of the MSA is thin on its face, and it manufactures findings an attorney could act on. **Headroom exists:** the full assembly was 76 343 chars ≈ 15 730 input tokens against `ollama_num_ctx` 32 768 — raising `msa_max_chars` to ~48 000 lands near ~20.7k tokens with output room to spare. Options, cheapest first: (a) raise the cap; (b) truncate on a **clause boundary** rather than mid-sentence; (c) state the truncation in the *directive* (top of the block, where it is read as an instruction) rather than only as a trailing marker; (d) retrieve MSA clauses relevant to the SOW instead of a blind head-slice. Shared by both surfaces via `skills/grounding.py::attach_parent_msa`, so the review path has the same exposure. Trace `85533de4`. |
| Word add-in: unsaved document silently resets all memory | Medium (new 2026-08-13) | [clients/word/src/docIdentity.ts](clients/word/src/docIdentity.ts) mints a UUID into `Office.context.document.settings`, which persists **with the file**. On an unsaved doc `saveAsync` has nowhere to write, so after a close/reopen `settings.get` returns empty and the client **mints a replacement** rather than returning `""`. The backend's preamble-hash fallback (`intake.py`: `state.get("document_id") or resolve_document_id(text)`) — which exists precisely for unsaved docs and *would* have matched across the reopen — never engages, because the client always has something to send. Observed live: reopening an unsaved NDA changed `document_id` `f873fbc3…` → `03fc1567…`, silently orphaning both the stored review and the per-attorney conversation. **No `memory_degraded` banner fires** — nothing failed, the key just changed. The file header's "Validated to survive a task-pane reopen" is accurate but narrower than it reads (pane reopen ≠ document reopen). Fix: return `""` when `Office.context.document.url` is empty so the content hash takes over. |
