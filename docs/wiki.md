...
> Last updated: 2026-06-16 | 198 tests + 43 frontend parser asserts passing | Python 3.12 | Redis checkpointer active | Word add-in shipped (chat-driven edits) | Trinetix legal-team playbook integrated | Smoke-tested on real MSA / SOW / BAA | Main-flow audit + bug fixes #1/#2/#3/#4/#5 (see docs/output_format_conflict.md)
...
| Deterministic placeholder / signature-block safety-net | High | **Content-quality recall gap (logged, not yet fixed).** On a real MSA (Langfuse `f4b383f0-f4f1-462e-af01-7931f285c828`) the review flagged 2 Preamble placeholders but **missed all 6 unfilled `[__]` signature-block placeholders** — even though the text contained them and §10.1 says "check signature blocks." Causes: two-column signature block (`Signed by: [__]\tSigned by: Boris …`) where the filled Trinetix column masks the empty client column; end-of-85KB-doc attention; inconsistent LLM recall. Fix: a deterministic scan of `uploaded_text` for unfilled markers (`[__]`, empty `[ ]`, template tokens) that guarantees each becomes a Missing Context blocker + forces "Do not send for signature", merged with the LLM findings (a first instance of the self-improving harness). |
