...
| **Server-side per-attorney conversation store (slice 2)** | `feat/conversation-store` | Chat conversations persist per `(document_id, attorney_id)` in SQLite (`memory/conversation_store.py`), appended per turn by `memory_writer` and read by `_run_doc_chat` (prefers the durable store over Redis `chat_history`, falls back when empty). Per-install attorney id from `localStorage` via the `X-User-ID` header (`clients/word/src/attorneyIdentity.ts`). Additive to Redis (checkpointer still owns interrupt/resume). +16 backend tests (290 total). **Smoke: pending** human sideload. |
