...
| ~~Chat: reconcile a recalled review against the current doc (stale-recall)~~ | ~~Medium~~ | **DONE** — shipped in `feat/stale-recall-reconciliation` (deterministic placeholder reconciliation; see the Shipped row). `_load_prior_review_block` now reconciles the recalled review against the current `uploaded_text` and drops findings whose placeholders were filled after the review, conservatively (never drops a live blocker). **Deferred (not in this branch):** the *re-review nudge* when the doc diverges beyond blank-fills (needs a doc-change signal), gate-verdict recomputation, and reconciling *substantive* findings via clause-level retrieval (needs structured-JSON review output). |
