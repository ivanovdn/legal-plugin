...
## Related system-prompt cleanups (change log)

**#3 — prompt noise (2026-06-16, branch `fix/prompt-noise-cleanup`).** Two assembly-layer
cleanups, source untouched:
- **Build-notice artifacts.** `skills/base.py::_clean_for_prompt` (via `_strip_leading_comments`)
  strips the leading `<!-- GENERATED by … -->` notice from each file at bundle-assembly time
  (~24 comment lines/review). Files on disk keep the notice for humans. *Rollback:* drop
  `_strip_leading_comments` from `_clean_for_prompt`. (Needs a backend restart — it's Python.)
- **Dangling `references/*.md` pointers.** `build_playbook.py::_rewrite_reference_pointers`
  rewrites the 3 per-type `SKILL.md` pointers ("Use `references/…`") to "…in this playbook",
  since the LLM has no file access and that content is already inline. *Rollback:* delete
  `_REFERENCE_REWRITES` + its use, rebuild. *Flag legal:* the source `SKILL.md` assumes the
  `references/` files are readable at runtime; our delivery inlines everything.
