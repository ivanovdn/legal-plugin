"""DOCX parser — ported from compliance-bot with LegalChunk output."""
...
def extract_label(text: str) -> str | None:
    """Extract 'Label:' prefix from List Paragraph items."""
...
def _is_bold_heading(para, num_info) -> bool:
    """Detect bold-as-heading pattern."""
...
def _table_to_text(table) -> str:
    """Convert a DOCX table to text rows."""
    rows = table.rows
    if not rows:
        return ""
...
def parse_docx(
    ...
    """Main parser. Returns flat list of LegalChunk objects."""
    ...
    for element in doc.element.body:
        if element.tag == qn("w:p"):
            ...
            if num_info and num_info["numFmt"] == "decimal":
                resolved = num_info["resolved"]
                clean_number = resolved.rstrip(".")
                text = f"{resolved} {text}"
                num_depth = len(clean_number.split("."))

                if num_depth == 1:
                    flush_all()
                    section_name = extract_clause_name(para)
                    if not section_name:
                        raw = para.text.strip()
                        if len(raw.split()) > 8:
                            for sep in [":", ",", "."]:
    ...
    # Filter noise chunks
    NOISE_SECTIONS = {
        "revision history", "change history", "table of contents",
        "document control", "version control", "approval history",
    }
